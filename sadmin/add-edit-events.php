...
if($_GET["editid"]!='' && $_GET["editid"]!=0 && is_numeric($_GET["editid"]))
{
	...
	if($resNews)
	{
		while($rowNews=mysql_fetch_array($resNews))
		{
			...
			$resourceFile1=trim($rowNews["resourceFile1"]);
...
if(isPost())
{
		...
		if(trim($errorMsg)=='')
		{
					if(!empty($_FILES["txtResourceFile1"]["name"]))
					{
						$extArray=strtolower(end(explode(".", $_FILES["txtResourceFile1"]["name"])));
						if($extArray=='png' || $extArray=='jpg' || $extArray=='jpeg' || $extArray=='gif')
						{
							$size=filesize($_FILES['txtResourceFile1']['tmp_name']);
							...
							{
								$resourceFile1=rand(0,99).date("Ymdsi");
								$resourceFile1=$resourceFile1.".".$extArray;
								
									
								$filePath=$_FILES['txtResourceFile1']['tmp_name'];
								$imageUploaded=false;
								chmod("../uploads/", 0777);  // octal; correct value of mode
								$imageUploaded=uploadImage($filePath,"../uploads/".$resourceFile1);
								chmod("../uploads/", 0755);  // octal; correct value of mode
								...
								{
									if(trim($_POST["txtOldFile1"])!='' && is_file("../uploads/".$_POST["txtOldFile1"]))
										unlink("../uploads/".$_POST["txtOldFile1"]);
								}
							}
						}
						else
					...
					{
							$resourceFile1=$_POST["txtOldFile1"];
					}
					else
					{
						//$errorMsg="Please upload .png, .jpg, .jpeg, .gif, files only.";
	...
	if(trim(strtolower($action))=="add")
	{
			...
			$sqlURLcheck="";
			$sqlURLcheck="Select resourceUrl from "._RESOURCES_TABLE_." where pageType='events' and resourceUrl='".$resourceUrl."' ";
	...
	else if(trim(strtolower($action))=="edit")
	{
		...
		$sqlURLcheck="";
		$sqlURLcheck="Select resourceUrl from "._RESOURCES_TABLE_." where pageType='events' and resourceUrl='".$resourceUrl."' and resourceId!=".$resourceId." ";
		$resURLcheck=getRecords(_RESOURCES_TABLE_,$selectFields,$whereFields,$whereVals,_Y_,$sqlURLcheck);
		if($resURLcheck)
		{
			$errorMsg="URL '".$resourceUrl."' already exists in database. Please enter another URL";
		...
		if(trim($errorMsg)=='')
		{
			...
			header("location:list-events.php?success=2");
			exit();
...
<title><?php echo ucfirst($action); ?>  Events</title>
  ...
  <tr>
    ...
    <td width="90%" align="left" valign="top"><div id="rightbody">
        ...
        <div class="rightbodysec">
          <h1><?php echo ucfirst($action); ?> Events</h1>
        </div>
        <div class="rightbodysec">
                  ...
                  <table width="100%" border="0" cellpadding="0" cellspacing="0"  class="mainbordersec">
					...
					CKFinder.setupCKEditor( editor,'<?php echo _CKFINDER_PATH_; ?>' ) ;
							...
							<tr>
                              ...
                              <?php if ($action=="edit" && $resourceFile1!=''){ ?>
                                
							<tr>
                              <td align="left" valign="top" style="padding:5px;"><span class="red_star"></span>Uploaded Image</td>
                              <td align="left" valign="top" style="padding:5px;"><img src="../uploads/<?php echo trim($resourceFile1); ?>" height="100" width="150"></td>
                            </tr>
                            <?php } ?>
							<tr>
                              <td align="left" valign="top" style="padding:5px;"><span class="red_star"></span>Upload Image</td>
                              <td align="left" valign="top" style="padding:5px;"><div class="erorfldcls" id="fld_txtResourceFile"></div>
                                <input name="txtResourceFile1" type="file" id="txtResourceFile1"  accept=".png, .jpg, .jpeg, .gif" onchange="hideerrordiv(this.id);" />
                               </td>
                            </tr>
                          </table>
                        </td>
                    ...
                    <tr>
                      <td align="left" valign="top" style="padding:5px;"><input name="txtAction" type="hidden" id="txtAction" value="<?php echo $action; ?>" />
                        ...
                        <input name="txtOldFile" type="hidden" id="txtOldFile" value="<?php echo $resourceFile; ?>" /> 
						   <input name="txtOldFile1" type="hidden" id="txtOldFile1" value="<?php echo $resourceFile1; ?>" />          </td>
                      <td align="left" valign="top" style="padding:5px;"><input name="Submit2" type="submit" class="bluebutton" value="<?php echo $btnValue; ?>" />
                        &nbsp;&nbsp;
                        <input name="button" type="button" class="bluebutton" value="<< Back"  onclick="javascript: window.location.href='list-events.php';"/>                      </td>
                    </tr>
                  </table>
                </form></td>
            </tr>
