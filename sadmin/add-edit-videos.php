...
if($_GET["editid"]!='' && $_GET["editid"]!=0 && is_numeric($_GET["editid"]))
{
	...
	$resVideos=getRecords(_RESOURCES_TABLE_,$selectFields,$whereFields,$whereVals,_Y_,$sqlEdit);
	if($resVideos)
	{
		while($rowVideos=mysql_fetch_array($resVideos))
		{
			$resourceId=trim($rowVideos["resourceId"]);
			$resourceTitle=$rowVideos["resourceTitle"];
			$resourceFile=trim($rowVideos["resourceFile"]);
			$shortDescription=trim($rowVideos["shortDescription"]);
			$description=$rowVideos["description"];
			$pageType=trim($rowVideos["pageType"]);
			$videoLink=$rowVideos["videoLink"];
		}
		$btnValue='Update';
		$action="edit";
	}
...
if(isPost())
{
		...
		if(trim($videoLink)=='')
		{
			$errorMsg="Please enter video url !";
		}
	...
	if(trim(strtolower($action))=="add")
	{
		...
		{							
				...
				header("location:list-videos.php?success=1");
				exit();					
	...
	else if(trim(strtolower($action))=="edit")
	{
		...
		if(trim($errorMsg)=='')
		{
			...
			header("location:list-videos.php?success=2");
			exit();
...
<title><?php echo ucfirst($action); ?> Videos</title>
  ...
  <tr>
    ...
    <td width="90%" align="left" valign="top"><div id="rightbody">
        ...
        <div class="rightbodysec">
          <h1><?php echo ucfirst($action); ?> Videos</h1>
        </div>
        <div class="rightbodysec">
              ...
              <td width="28%" align="left" valign="top"><form action="" method="post" enctype="multipart/form-data" name="frmResources" id="frmResources">
                  <table width="100%" border="0" cellpadding="0" cellspacing="0"  class="mainbordersec">
					...
					CKFinder.setupCKEditor( editor,'<?php echo _CKFINDER_PATH_; ?>' ) ;
							...
							<tr>
                              <td align="left" valign="top" style="padding:5px;"><span class="red_star">*</span>Video URL </td>
                              <td align="left" valign="top" style="padding:5px;"><div class="erorfldcls" id="fld_videoLink"></div>
                                <input name="videoLink" type="text" id="videoLink" class="fiellength" onkeyup="hideerrordiv(this.id);"  placeholder="Youtube embed URL" value="<?php echo $videoLink; ?>"/>
                                <br />
                                e.g. https://www.youtube.com/watch?v=QPc4NSZ48sg </td>
                            </tr>
                    ...
                    <tr>
                      ...
                      <td align="left" valign="top" style="padding:5px;"><input name="Submit2" type="submit" class="bluebutton" value="<?php echo $btnValue; ?>" />
                        &nbsp;&nbsp;
                        <input name="button" type="button" class="bluebutton" value="<< Back"  onclick="javascript: window.location.href='list-videos.php';"/>                      </td>
                    </tr>
                  </table>
                </form></td>
            </tr>
